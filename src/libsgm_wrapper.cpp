...
LibSGMWrapper::LibSGMWrapper(int numDisparity, int P1, int P2, int Pd1, int Pd2, float alpha, float uniquenessRatio, bool subpixel, PathType pathType, int minDisparity, int lrMaxDiff, CensusType censusType, bool shift8)
	: sgm_(nullptr), numDisparity_(numDisparity), param_(P1, P2, Pd1, Pd2, alpha, uniquenessRatio, subpixel, pathType, minDisparity, lrMaxDiff, censusType, shift8), prev_(nullptr) {}
...
void LibSGMWrapper::execute(const cv::cuda::GpuMat& I1, const cv::cuda::GpuMat& I2,
	const cv::cuda::GpuMat& S1, 
	// const cv::cuda::GpuMat& S2,
	cv::cuda::GpuMat& disparity)
{
	const cv::Size size = I1.size();
	CV_Assert(size == I2.size());
	CV_Assert(size == S1.size());
	// CV_Assert(size == S2.size());

	CV_Assert(I1.type() == I2.type());
	// CV_Assert(S1.type() == S2.type());

	const int depth = I1.depth();
	CV_Assert(depth == CV_8U || depth == CV_16U || depth == CV_32S);
	const int depth2 = S1.depth();
	CV_Assert(depth2 == CV_8U || depth2 == CV_16U);
	...
	sgm_->execute(I1.data, I2.data, S1.data, disparity.data);
}

void LibSGMWrapper::execute(const cv::Mat& I1, const cv::Mat& I2, const cv::Mat& S1, cv::Mat& disparity)
{
	const cv::Size size = I1.size();
	CV_Assert(size == I2.size());
	CV_Assert(size == S1.size());
